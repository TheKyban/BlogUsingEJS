...
app.get("/contact", (req, res) => {
  res.render("contact", { contactContact: contactContent })
})
...
app.get("/posts/:days", (req, res) => {
  const parameter = req.params.days.toLowerCase().split("%20") //a
  console.log(parameter)
  posts.forEach((post, index) => {

    const title = post["title"] //a a
    if (parameter.length === 1) {
      if (title === parameter[0]) {
        res.render("post", { title: post["title"], content: post["content"] })
      }
    } else if (title === parameter.join(" ")) {
      res.render("post", { title: post["title"], content: post["content"] })
